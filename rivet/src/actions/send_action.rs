...
pub fn send_request(path: &str) -> Result<ApiResponse, String> {
    if let Ok(current_path) = env::current_dir() {
        ...
        // Check if we can load the dotenv file if exists
        if env_path.exists() {
        ...
        // Convert the raw string content to toml structure for Rust
        let mut request_config: RequestConfig = match toml::from_str(&file_content) {
            Ok(config) => config,
            Err(error) => {
                return Err(format!("Failed to parse your TOML file: {}", error.red()));
            }
        };
        ...
        match request.send() {
            Ok(response) => {
                ...
                match response.text() {
                    ...
                    Err(error) => {
                        return Err(format!("Failed to decode response body: {}", error.red()));
                    }
                }
            }

            Err(error) => return Err(format!("Failed to send request: {}", error.red())),
        }
    }
    return Err(format!("{}", "Cannot get current directory!".red()));
}
